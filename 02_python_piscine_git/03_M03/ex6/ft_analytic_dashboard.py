...
def list_comprehension():
    ...
    high_score = [player for player, score in zip(players, scores) if score > 2000]
...
def dict_comprehension():
    ...
    value = {
        ...
        "medium": len([score for score in scores if score < 2000 and score > 1500]),
        "low": len([score for score in scores if score < 1500])
        }
    print(f"Score categories: {value}")
    ach = {name: len(achievement) for name, achievement in achievements.items()}
...
def set_comprehension():
    ...
    unique_achievement = {achievement for achievement_list in achievements.values() for achievement in achievement_list}
